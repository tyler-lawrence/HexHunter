...
class GameCenterManager {
    ...
    typealias EntryResult = (GKLeaderboard.Entry?, [GKLeaderboard.Entry])
    ...
    func fetchLeaderboardEntries(for gameMode: GameMode) async -> [GKLeaderboard.Entry] {
        ...
        do {
            ...
            if let modeLeaderboard = leaderboards.first(where: {$0.baseLeaderboardID == leaderboardID}) {
                let leaderboardEntries = try await modeLeaderboard.loadEntries(
                    for: .global,
                    timeScope: .allTime,
                    range: NSRange(1...100)
                )
    ...
    func fetchPlayerLeaderboardEntry(for gameMode: GameMode) async -> GKLeaderboard.Entry? {
        ...
        do {
            let leaderboards  = try await GKLeaderboard.loadLeaderboards(IDs: [leaderboardID])
            if let modeLeaderboard = leaderboards.first(where: {$0.baseLeaderboardID == leaderboardID}) {
                let fetchedEntry: EntryResult = try await modeLeaderboard.loadEntries(
                    for: [localPlayer],
                    timeScope: .today
                )
