...
struct HexcodeExplanationView: View {
    ...
    Let's explore how we can convert something like #\(hexcode.display) to the 0-255 scale.
    """
    }
    ...
    The first number is a large scale change and the second character is finer detail
    """
    let conversionIntroMessage =
    """
    To convert the hexcode to a number you first convert the digit\
    to its corresponding numeric value (if necessary) and apply the following formula:
    """
}
